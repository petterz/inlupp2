...
bool shelf_check(list_t *shelf_list, tree_t *lager, char *shelf){
  if(search_shelves(shelf_list, shelf)){
    return true;
  }
...
char *ask_question_shelf(list_t *current_item_shelves, tree_t *lager){
  char *question = "Lägg till hyllnummer";
  char *input = ask_question(question, is_shelf, (convert_func) strdup).s;
  while(1){
    if(!shelf_check(current_item_shelves, lager, input)){
      break;
    }
...
void input_shelf(list_t *current_item_shelves, tree_t *lager){
  do{
    shelf_t *shelf = calloc(1, sizeof(shelf_t));
    char *shelf_name = ask_question_shelf(current_item_shelves, lager);
    shelf->shelf = shelf_name;
    shelf->amount = ask_question_int("Lägg in antal");
    list_append(current_item_shelves, shelf); //append to real list
...
void edit_goods(tree_t *lager, action_t *undo){
  ...
  void *get = tree_get(lager, key);
  item_t *item = (item_t *)get;
  list_t *copy_shelves = list_new();
  
  for(int i = 0; i < list_length(item->shelfs); ++i){
    shelf_t *og_shelf = list_get(item->shelfs, i);
    shelf_t *shelf = calloc(1, sizeof(shelf_t));
    shelf->shelf = og_shelf->shelf;
    shelf->amount = og_shelf->amount;
    list_append(copy_shelves, shelf);
  }
  undo->type = 3;
  undo->copy = *item;
  undo->merch = item;
  undo->shelves = copy_shelves;
...
void undo_edit_aux(item_t *ex_item, item_t og_item, list_t *shelves){
  ex_item->desc = og_item.desc;
  ex_item->price = og_item.price;
  ex_item->shelfs = shelves;
}

void undo_edit(tree_t *lager, action_t *undo){
  undo_edit_aux(undo->merch, undo->copy, undo->shelves);
  undo->type = 0;
}
